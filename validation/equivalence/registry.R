...
.equivalence_markdown <- function(run) {
  ...
  header <- c(
    "# Equivalence report",
    "",
    sprintf("Generated by `Rscript validation/equivalence/run.R` against **multilpa %s** under %s on %s.",
    ...
    if (startsWith(equivalence_provenance(run, "coverage"), "PARTIAL")) {
      sprintf("**This report covers only part of the harness: %s.** Run `Rscript validation/equivalence/run.R` with no arguments to regenerate complete evidence.",
