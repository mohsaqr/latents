...
| `latentgold/` | Latent GOLD 6.1 comparison, **run 2026-09-22: 138 quantities compared, 138 agree, 0 disagree** across eleven cases (two-level continuous, full covariance, categorical, mixed, FIML, covariates, bivariate residuals, three-step, latent transitions). Latent GOLD's own listings and posterior files are retained in `latentgold/returned/` with `PROVENANCE.md` and an input fingerprint, so `compare.R` runs offline and Latent GOLD is not needed again unless the kit changes. Standard errors, bivariate residuals and the Step-3 tables are still `awaiting parser`. |
