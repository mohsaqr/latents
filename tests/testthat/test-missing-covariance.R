test_that("Gaussian missing-pattern moments agree with conditional normal formulas", {
  ...
  moments <- .ml_lpa_gaussian_moments(x, parameters)
  ...
  independent <- .ml_lpa_gaussian_moments(x, diagonal)
  expect_equal(independent$moments[[1L]]$expected,
               rbind(c(3, 2), c(1, 5), c(1, 2), c(3, 5)))
  expect_error(.ml_lpa_gaussian_moments(matrix(Inf, 1L), parameters))
})
...
test_that("diagonal FIML single-profile estimates equal available-case Gaussian ML", {
  ...
  fit <- fit_ml_lpa(data, c("x", "y"), "group", 1, 1,
  ...
  expect_error(fit_ml_lpa(data, c("x", "y"), "group", 1, 1), "missing")
})
...
test_that("full covariance complete-data Gaussian ML has analytic means and covariance", {
  ...
  one <- fit_ml_lpa(data, "x", "group", 1, 1,
                    covariance_model = "full", n_starts = 1)
...
test_that("full covariance mixture limits reproduce mclust VVV and EEE", {
  ...
  invisible(lapply(c(varying = "VVV", equal = "EEE"), function(model) {
    ...
    fit <- fit_ml_lpa(data, c("x", "y"), "group", 2, 1, covariance_model = "full",
...
test_that("FIML multilevel rows use their observed group context", {
  ...
  invisible(lapply(c("equal", "varying"), function(variance_model) {
    fit <- fit_ml_lpa(data, c("x", "y"), "group", 2, 2, missing = "fiml",
...
test_that("covariance eigenvalue bounds and invalid missing-data inputs are explicit", {
  ...
  bounded <- .ml_lpa_bound_covariance(covariance, .1)
  expect_equal(eigen(bounded, symmetric = TRUE)$values, c(1.99, .1))
  expect_equal(.ml_lpa_bound_covariance(covariance, .001), covariance)
  expect_true(.ml_lpa_covariance_boundary(list(means = matrix(0, 1, 2),
    covariances = array(bounded, c(2, 2, 1))), .1))
  expect_error(.ml_lpa_bound_covariance(matrix(NA_real_, 1), .1))
  data <- data.frame(group = rep(1:5, each = 4), x = seq_len(20), y = seq_len(20))
  expect_warning(fit <- fit_ml_lpa(data, c("x", "y"), "group", 1, 1,
    n_starts = 1, covariance_model = "full", min_variance = .1), "bound-active")
  expect_true(fit$boundary)
  expect_true(fit$starts$boundary)
  data$x <- NA_real_
  expect_error(fit_ml_lpa(data, c("x", "y"), "group", 1, 1,
  ...
  valid <- .ml_lpa_validate_start(start, 1, 1, 2, "varying", .1, "full")
  expect_equal(valid$variances, matrix(c(1, 1), 1))
  start$covariances[1, 2, 1] <- .2
  expect_error(.ml_lpa_validate_start(start, 1, 1, 2, "varying", .1, "full"), "symmetric")
  start$covariances <- array(diag(2), c(2, 2, 1))
  start$variances <- matrix(c(2, 2), 1)
  expect_error(.ml_lpa_validate_start(start, 1, 1, 2, "varying", .1, "full"), "diagonals")
  start$covariances <- diag(2)
  expect_error(.ml_lpa_validate_start(start, 1, 1, 2, "varying", .1, "full"), "array")
})
test_that("FIML rejects NaN consistently with inference", {
  data <- data.frame(group = rep(1:3, each = 2), y = c(1, 2, NaN, 4, 5, 6))
  expect_error(fit_ml_lpa(data, "y", "group", 1, 1, missing = "fiml"), "non-finite")
})
