test_that("enumeration retains failures and both BIC conventions", {
  ...
  result <- enumerate_multilpa(d, "y", "g", profiles = 1:2, group_classes = 1:2,
                             n_starts = 2, seed = 12)
  ...
  expect_error(enumerate_multilpa(d, "y", "g", profiles = 0), "profiles")
  expect_error(enumerate_multilpa(d, "y", "g", n_profiles = 2), "model counts")
})
...
test_that("simulation preserves group layout and full covariance moments", {
  ...
  model <- fit_multilpa(d, c("a", "b"), "g", 1, 1, covariance_model = "full", n_starts = 1)
  ...
  one <- fit_multilpa(d, "a", "g", 1, 1, covariance_model = "full", n_starts = 1)
...
test_that("bootstrap withholds p-values if a replicate cannot be fitted", {
  ...
  small <- fit_multilpa(d, "y", "g", 1, 1, n_starts = 2, seed = 12)
  large <- fit_multilpa(d, "y", "g", 2, 1, n_starts = 2, seed = 12)
  testthat::local_mocked_bindings(fit_multilpa = function(...) stop("test optimization failure"))
  expect_warning(result <- bootstrap_lrt_multilpa(small, large, d, n_boot = 2, seed = 1), "p_value is NA")
...
test_that("bootstrap refits generated data and reports finite simulation correction", {
  ...
  small <- fit_multilpa(d, "y", "g", 1, 1, variance_model = "equal", n_starts = 2, seed = 8)
  large <- fit_multilpa(d, "y", "g", 2, 1, variance_model = "equal", n_starts = 3, seed = 8,
                      max_iter = 3000, tol = 1e-7)
  rng <- .Random.seed
  result <- bootstrap_lrt_multilpa(small, large, d, n_boot = 3, n_starts = 3,
  ...
  expect_error(bootstrap_lrt_multilpa(small, small, d, n_boot = 2), "one class")
  changed <- d
  changed$y <- d$y + 1
  expect_error(bootstrap_lrt_multilpa(small, large, changed, n_boot = 2), "reproduce")
  changed$y[1] <- NA_real_
  expect_error(bootstrap_lrt_multilpa(small, large, changed, n_boot = 2), "complete")
})
...
test_that("full covariance print and summary expose residual matrices", {
  ...
  fit <- fit_multilpa(d, c("a", "b"), "g", 1, 1, covariance_model = "full", n_starts = 1)
