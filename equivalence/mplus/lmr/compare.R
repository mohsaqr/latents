...
# Run from the project root: Rscript equivalence/mplus/lmr/compare.R
suppressMessages(pkgload::load_all(".", quiet = TRUE))
source(file.path("equivalence", "fixtures.R"))
artifact_dir <- file.path("equivalence", "mplus", "lmr")
...
fitted_null <- multilpa(fixture$data, indicators, "cluster", 1L, 1L,
                          variance_model = "equal", n_starts = 5L,
                          max_iter = 5000L, tol = 1e-12, seed = 912)
fitted_alternative <- multilpa(fixture$data, indicators, "cluster", 2L, 1L,
...
source(file.path("equivalence", "deferred.R"))
